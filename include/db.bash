...
db-dump() {
    ...
    if [ "$dbName" = "all" ]; then
        for db in $CB_DB_ENV_DB $PERISCOPE_DB_ENV_DB $VAULT_DB_SCHEMA $DATALAKE_DB_ENV_DB $REDBEAMS_DB_ENV_DB $ENVIRONMENT_DB_ENV_DB $FREEIPA_DB_ENV_DB $IDBMMS_DB_ENV_DB $WORKLOADIAM_DB_ENV_DB $CLUSTER_PROXY_DB_ENV_DB $ULU_DB_ENV_DB; do
            db-dump-database $db
        done
    else
...
db-initialize-databases() {
    ...
    for db in $CB_DB_ENV_DB $PERISCOPE_DB_ENV_DB $VAULT_DB_SCHEMA $DATALAKE_DB_ENV_DB $REDBEAMS_DB_ENV_DB $ENVIRONMENT_DB_ENV_DB $FREEIPA_DB_ENV_DB $IDBMMS_DB_ENV_DB $WORKLOADIAM_DB_ENV_DB $CLUSTER_PROXY_DB_ENV_DB $AUDIT_DB_ENV_DB $DATALAKE_DR_DB_ENV_DB $ULU_DB_ENV_DB; do
