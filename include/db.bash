...
db-dump() {
    ...
    if [ "$dbName" = "all" ]; then
        for db in $CB_DB_ENV_DB $IDENTITY_DB_NAME $PERISCOPE_DB_ENV_DB $VAULT_DB_SCHEMA $DATALAKE_DB_ENV_DB $REDBEAMS_DB_ENV_DB $ENVIRONMENT_DB_ENV_DB $FREEIPA_DB_ENV_DB; do
            db-dump-database $db
        done
    else
